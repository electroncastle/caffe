...
void CropLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < 2; ++i) {
    for (Blob<Dtype>* blob = bottom[i]; blob != inter_blob;
      ...
      DiagonalAffineMap<Dtype> cords = layer->coord_map();
     vector<pair<Dtype, Dtype> > cofs = cords.coefs();
     std::cout << "Layer: " << layer->layer_param().name() << std::endl << std::flush;
     for (int x=0; x<cofs.size(); x++){
         std::cout << "Stride: " << cofs[x].first << "  " << cofs[x].second << std::endl << std::flush;
     }

      coord_maps[i] = coord_maps[i].compose(cords);
    }
  }
